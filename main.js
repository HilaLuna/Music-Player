...
//prevButton.style.backgroundColor = "red" //! bununla o prev butonuna eriştiğimizi görmüş olduk.
...
//Şarkı atama
const setSong = (arrayIndex) =>{ //burdaki setSong ismini biz verdik
    ...
    audio.src = link //audio'da ses atamasını src ile yapıyoruz. Resimde de src kullanıyoruz, diğerlerinde innerHTML veya innerText.
    songName.innerHTML = name //bu iki üstteki süslü parantezin içindeki name, link de öyle.
...
const nextSong = () =>{
    if (loop) {
        if (index == (songsList.length -1)) { //şarkı listesinin uzunluğu 5 şarkı olduğu için toplamdan -1 olunca, 4 şarkı oluyo. 
            index = 0
        } else {
            index +=1
        }
...
shuffleButton.addEventListener('click', ()=>{
    if(shuffleButton.classList.contains('active')) {
        ...
        audio.loop = true
        console.log('kariştirici kapatildi')
    } else {
        isShuffleActive = true
        shuffleButton.classList.add('active')
        audio.loop = false
        console.log('kariştirici acildi')
